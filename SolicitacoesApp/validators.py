...
def validate_nome_completo(value):
    ...
    if valid_substrings < 2: 
        raise ValidationError(ERROR_MESSAGES['nome_invalido']) # Chama o erro
...
def validate_min_30(value):
    ...
    if valid_substrings < 2 or len(value) < 30 or len(value) > 250:  
        raise ValidationError(ERROR_MESSAGES['min_caracteres']) # Chama o erro

def validate_data_futuro(value):
    if value and value < datetime.now().date():
        raise ValidationError(ERROR_MESSAGES['data_no_passado'])
